...
public class AddAdmin extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int adminID = (int) (Math.random() * (9999-1000));
		...
		System.out.println("ADDCUSTOMER " + adminID + admin_name);
			...
			if(rs.next()) {
				...
				try{
					...
					if(rows>0) {
						...
						ses.setAttribute("userid", adminID);
						ses.setAttribute("pass", password);
						System.out.println("Admin added in db");
						System.out.println();
